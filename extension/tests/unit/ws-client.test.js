...
describe('WebSocket client', () => {
  it('(+) connect sends auth token', async () => {
    let ws;
    const origWS = globalThis.WebSocket;
    globalThis.WebSocket = class extends MockWebSocket {
      constructor(...args) { super(...args); ws = this; }
    };
    connect({ url: 'http://127.0.0.1:9876', token: 'test-tok' });
    await new Promise((r) => setTimeout(r, 20));
    expect(ws.sent[0]).toEqual({ type: 'auth', token: 'test-tok' });
  ...
  it('(+) sendAnnotationCreate sends correct message', async () => {
    ...
    connect({ url: 'http://127.0.0.1:9876', token: 'tok' });
    await new Promise((r) => setTimeout(r, 20));
    sendAnnotationCreate({ id: 'a1', comment: 'test' });
    expect(ws.sent[1]).toEqual({ type: 'annotation:create', annotation: { id: 'a1', comment: 'test' } });
    globalThis.WebSocket = origWS;
  });
  ...
  it('(+) sendAnnotationUpdate sends uuid and changes', async () => {
    ...
    expect(ws.sent[1]).toEqual({ type: 'annotation:update', uuid: 'u1', changes: { comment: 'updated' } });
    globalThis.WebSocket = origWS;
  });
  ...
  it('(+) sendAnnotationDelete sends uuid', async () => {
    ...
    expect(ws.sent[1]).toEqual({ type: 'annotation:delete', uuid: 'u1' });
    globalThis.WebSocket = origWS;
  });
