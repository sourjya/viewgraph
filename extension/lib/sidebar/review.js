...
import { trashIcon, cameraIcon, closeIcon, checkIcon, circleIcon, setSvg } from './icons.js';
...
function renderFilterTabs(tabContainer, { open, resolved, anns, activeFilter, activeTypeFilters, serverResolvedHistory }, callbacks) {
  ...
  const filterTypes = [
    ...
    { key: 'idea', label: 'Ideas', color: 'var(--vg-color-highlight)' },
    { key: 'diagnostic', label: 'Diagnostics', color: 'var(--vg-color-diagnostic)' },
    { key: 'page-note', label: 'Notes', color: 'var(--vg-color-info)' },
  ];
...
function showClearConfirmation(count, callbacks) {
  ...
  Object.assign(card.style, {
    background: 'var(--vg-surface-card)', border: `1px solid ${COLOR.error}`, borderRadius: '10px',
  ...
  Object.assign(msg.style, { color: 'var(--vg-color-text)', fontSize: '13px', fontWeight: '600', marginBottom: '4px' });
...
function createRequestEntry(req, callbacks) {
  ...
  Object.assign(capBtn.style, {
    ...
    background: COLOR.warning, color: 'var(--vg-color-black)', display: 'flex', cursor: 'pointer',
  });
  ...
  if (req.guidance) {
    ...
    Object.assign(guide.style, {
      color: 'var(--vg-color-text)', fontSize: '12px', lineHeight: '1.4',
      marginTop: '6px', padding: '6px 8px',
      background: 'var(--vg-surface-elevated)', borderRadius: '0', borderLeft: `3px solid ${COLOR.warning}`,
    });
  ...
  for (const ann of history) {
    ...
    if (ann.ancestor) {
      ...
      Object.assign(elBadge.style, {
        background: COLOR.bgHover, color: 'var(--vg-color-info-light)', fontSize: '10px', fontWeight: '500',
...
const SEV_DOT_COLORS = { critical: 'var(--vg-color-error-dim)', major: 'var(--vg-color-highlight)', minor: 'var(--vg-color-text-muted)' };
...
function createEntry(ann, callbacks) {
  ...
  if (ann.severity) {
    ...
    Object.assign(sevIcon.style, {
      background: SEV_DOT_COLORS[ann.severity] || 'var(--vg-color-severity)',
  ...
  if (isDiagnostic) {
    ...
    Object.assign(sectionTag.style, { background: 'var(--vg-color-info-bg)', color: 'var(--vg-color-info)', padding: '0 4px', borderRadius: '3px', fontSize: '9px', marginRight: '4px' });
