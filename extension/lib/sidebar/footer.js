...
export function createFooter({ onSend, onShowSettings }) {
  ...
  addHover(sendBtn, 'var(--vg-color-badge)', COLOR.primary);
  ...
  function flashButton(btn, msg, iconFn, label) {
    ...
    btn.style.background = 'var(--vg-color-success-dim)';
  ...
  function setOfflineMode() {
    ...
    Object.assign(dlBtn.style, { background: 'var(--vg-border-subtle)', color: COLOR.white, border: 'none', flex: '1' });
  }
  ...
  function flashSend() {
    ...
    sendBtn.style.background = 'var(--vg-color-success-dim)';
  ...
  const TRUST_COLORS = { trusted: COLOR.success, configured: 'var(--vg-color-info)', untrusted: COLOR.warning };
  ...
  function setTrustLevel(trust) {
    ...
    trustShield.setAttribute('data-tooltip', `${trust.level[0].toUpperCase() + trust.level.slice(1)}: ${trust.reason}`);
