...
export function createWebSocketServer(httpServer, options = {}) {
  // 13.10 + 15.6: WS maxPayload is intentionally lower than HTTP MAX_BODY (5MB).
  // WS carries only events (annotation status, audit results, capture notifications).
  // Full captures go through HTTP POST /captures. 1MB is sufficient for events.
