/**
 * get_annotation_context - MCP Tool Integration Tests
 */
...
import { register } from '#src/tools/get-annotation-context.js';

const FIXTURES_DIR = path.resolve(import.meta.dirname, '../../fixtures');

describe('get_annotation_context via MCP', () => {
  let cleanup;
  ...
  it('returns filtered capture with annotation comments', async () => {
    ...
    cleanup = c;
    const result = await client.callTool({ name: 'get_annotation_context', arguments: { filename: 'annotated-capture.json' } });
  ...
  it('filters to single annotation by id', async () => {
    const { client, cleanup: c } = await createTestClient((s) => register(s, createIndexer({ maxCaptures: 10 }), FIXTURES_DIR));
    cleanup = c;
    const result = await client.callTool({ name: 'get_annotation_context', arguments: { filename: 'annotated-capture.json', annotation_id: 'ann-2' } });
    const parsed = JSON.parse(result.content[0].text);
        const output = parsed.annotatedNodes || parsed;
  ...
  it('returns message for non-annotated capture', async () => {
    const { client, cleanup: c } = await createTestClient((s) => register(s, createIndexer({ maxCaptures: 10 }), FIXTURES_DIR));
    cleanup = c;
    const result = await client.callTool({ name: 'get_annotation_context', arguments: { filename: 'valid-capture.json' } });
