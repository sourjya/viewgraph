...
ViewGraph is an open-source browser extension + MCP server that provides UI context to AI coding agents. The extension captures structured DOM snapshots from any web page (elements, styles, selectors, accessibility state, network errors) and a local MCP server exposes them to AI assistants via the Model Context Protocol. All data stays on the user's local machine - no cloud services, no external APIs, no telemetry. Users are developers, testers, QA reviewers, and non-technical stakeholders working on web applications. The tool is distributed via Chrome Web Store, Firefox Add-ons, npm (@viewgraph/core), and GitHub releases. Current version 0.4.2 with 37 MCP tools, 17 enrichment collectors, and 1571 tests. Communication between extension and server is via localhost HTTP (ports 9876-9879) with native messaging as a secondary path. The server runs on Node.js 22+ and communicates with AI agents via stdio (MCP protocol).
...
**Description**: ViewGraph is an open-source browser extension + MCP server that provides UI context to AI coding agents. The extension captures structured DOM snapshots from any web page (elements, styles, selectors, accessibility state, network errors) and a local MCP server exposes them to AI assistants via the Model Context Protocol. All data stays on the user's local machine - no cloud services, no external APIs, no telemetry. Users are developers, testers, QA reviewers, and non-technical stakeholders working on web applications. The tool is distributed via Chrome Web Store, Firefox Add-ons, npm (@viewgraph/core), and GitHub releases. Current version 0.4.2 with 37 MCP tools, 17 enrichment collectors, and 1571 tests. Communication between extension and server is via localhost HTTP (ports 9876-9879) with native messaging as a secondary path. The server runs on Node.js 22+ and communicates with AI agents via stdio (MCP protocol).
