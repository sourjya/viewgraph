...
- [ ] 1.1 Create `server/tests/fixtures/valid-capture.json` — minimal valid ViewGraph v2 capture with all sections
- [ ] 1.2 Create `server/tests/fixtures/annotated-capture.json` — capture with ANNOTATIONS section
- [ ] 1.3 Create `server/tests/fixtures/malformed-capture.json` — invalid JSON for error handling tests
  - _Requirements: FR-3.2_

#### Step 2: ViewGraph v2 Parser — TDD Cycle

**RED Phase:**
- [ ] 2.1 Write tests for `parseMetadata` — extracts metadata from valid capture, returns error for malformed
- [ ] 2.2 Write tests for `parseCapture` — parses all sections, handles missing sections gracefully
- [ ] 2.3 Write tests for `parseSummary` — extracts summary data for get_page_summary
  - File: `server/tests/unit/viewgraph-v2.test.js`
  - _Requirements: FR-3.1, FR-3.2, FR-3.3_

**GREEN Phase:**
- [ ] 2.4 Implement `server/src/parsers/viewgraph-v2.js`
...
**RED Phase:**
- [ ] 3.1 Write tests for indexer: add, remove, get, list (with limit/filter/sort), getLatest, eviction
  - File: `server/tests/unit/indexer.test.js`
  - _Requirements: FR-2.1, FR-2.2, FR-2.3, FR-2.4, FR-2.5_

**GREEN Phase:**
- [ ] 3.2 Implement `server/src/indexer.js`
...
- [ ] 4.1 Implement `server/src/watcher.js`
...
- [ ] 4.2 Write integration test: watcher + indexer together
...
- [ ] 5.1 Implement `server/src/utils/validate-path.js` — shared path validation
  - _Requirements: NFR-2_

#### Step 6: list_captures — TDD Cycle

**RED Phase:**
- [ ] 6.1 Write integration tests using InMemoryTransport
...
**GREEN Phase:**
- [ ] 6.2 Implement `server/src/tools/list-captures.js`
...
**GREEN Phase:**
- [ ] 7.2 Implement `server/src/tools/get-capture.js`
  - _Requirements: FR-6.1 through FR-6.4_
...
**GREEN Phase:**
- [ ] 8.2 Implement `server/src/tools/get-latest.js`
  - _Requirements: FR-5.1 through FR-5.4_
...
**GREEN Phase:**
- [ ] 9.2 Implement `server/src/tools/get-page-summary.js`
  - _Requirements: FR-7.1 through FR-7.3_
...
- [ ] 10.1 Update `server/index.js` to:
...
- [ ] 11.2 Verify all unit + integration tests pass: `npm run test:server`
